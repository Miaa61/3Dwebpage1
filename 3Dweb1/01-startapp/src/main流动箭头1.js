...
//     //加载纹理
//     arrowLineTexture = new THREE.TextureLoader().load('./texture/picture/arrowhead.png');
//     arrowLineTexture.wrapS = arrowLineTexture.wrapT = THREE.RepeatWrapping; //每个都重复
//     arrowLineTexture.repeat.set(1, 1); //水平重复1次
...
function initFlowingLine() {
    //加载纹理
    flowingLineTexture = new THREE.TextureLoader().load('./texture/picture/arrowhead.png')

    flowingLineTexture.wrapS = THREE.RepeatWrapping;
    flowingLineTexture.wrapT = THREE.RepeatWrapping;
    flowingLineTexture.repeat.set(1, 1); //水平重复20次
    ...
    let meshway = new THREE.Mesh(tubeGeometry, material);
    meshway.position.z = 0;
    scene.add(meshway);
}
initFlowingLine();
...
function render() {
    ...
    flowingLineTexture.offset.x -= 0.003; //更新流动线纹理偏移量
